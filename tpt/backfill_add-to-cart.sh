...
DAY='01'
RUN="$($DT_CMD +'%s')"
TABLE="tpt-data-warehouse-prod:dev_outbox.add_to_cart_backfill_$YEAR$MONTH$DAY"
GCS="gs://tpt_data_sci_dev/resources-recommendations/add-to-cart/backfill/$YEAR/$MONTH/$DAY/$RUN/*.json"
GCS_ERROR="gs://tpt_data_sci_dev/resources-recommendations-errors/"
SQL="$(jinja2 --strict -D dt=$YEAR-$MONTH-$DAY -D backfill=yes sql/add_to_cart.tpl.sql)"
